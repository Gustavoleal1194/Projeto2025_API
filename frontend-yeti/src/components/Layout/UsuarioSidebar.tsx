...
const UsuarioSidebar: React.FC<UsuarioSidebarProps> = ({ activeTab, setActiveTab, isCollapsed, onToggleCollapse }) => {
    return (
        <aside className={`fixed left-0 top-0 h-full bg-blue-900 text-white shadow-2xl z-50 flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-70'}`} style={{ width: isCollapsed ? '4rem' : '17.5rem' }}>
            {/* Logo Container */}
            {!isCollapsed && (
                <div className="p-8 text-center border-b border-blue-700" style={{ paddingTop: '48px' }}>
                    <div className="bg-blue-200 rounded-full mx-auto flex items-center justify-center border-4 border-blue-400 w-24 h-24" style={{ marginBottom: '16px' }}>
                        <svg
                            className="w-16 h-16"
            ...
            {/* Navigation */}
            <nav
                className={`flex-1 ${isCollapsed ? 'overflow-hidden p-2' : 'overflow-y-auto sidebar-scroll p-4'}`}
                style={{
                    scrollbarWidth: isCollapsed ? 'none' : 'thin',
                    scrollbarColor: isCollapsed ? 'transparent transparent' : '#93c5fd #1e3a8a',
                    paddingTop: isCollapsed ? '80px' : '16px'
                }}
            >
                {[
                    ...
                    <button
                        ...
                        className={`w-full flex items-center rounded-lg transition-all duration-300 group relative ${isCollapsed ? 'p-4 justify-center mb-3' : 'p-4 mb-2'} ${activeTab === item.id
                    ...
                    >
                        <span className={`${isCollapsed ? 'text-2xl' : 'text-2xl mr-3'}`}>{item.icon}</span>
                        {!isCollapsed && (
